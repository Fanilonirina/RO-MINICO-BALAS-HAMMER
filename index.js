$(function () {
    ...
    $("#calculGainBtn").click(function() {
        ...
        for(var i=0; i < baseSolutionTable.length; i++) {
            for(var j=0; j < baseSolutionTable[i].length; j++) {
                if(baseSolutionTable[i][j] == 0) {
                    ...
                    if(resultat >= 0) {
                        $("ul#listeGains").append('<li class="ml-3 text-center"> G('+ noeuds[i].key +', '+ noeuds[j+nombreSource].key +') = '+
                    ...
                    // Si il y a des valeurs négatives, nous allons calculer les gains
                        ...
                        if(coefficient == "E"){
                            ...
                            $("ul#listeGains").append('<li class="ml-3 text-danger text-center"> G('+ noeuds[i].key +', '+ noeuds[j+nombreSource].key +') = '+
                                noeuds[i].value + '+' + originalTable[i][j] + '-' + noeuds[j+nombreSource].value + 
        ...
        if(listChemin.length > 0) {
            ...
            $("#divInsert").html('<h5 class="text-center font-weight-bold text-uppercase py-2">Transport optimal</h5>');
            alert("TRANSPORT OPTIMALE");
        }
    ...
    $("#getGraphBtn").click(function() {
        ...
        diagram.nodeTemplate = $$(go.Node, "Auto",
                                    ...
                                    $$(go.Panel,
                                        $$(go.Shape, "Circle", { fill: "red", width: 60, height: 60 }), 
                                        $$(go.TextBlock,
                                            { margin: 2, width: 25 },
        ...
        diagram.linkTemplate = $$(go.Link,
                                    { curve: go.Link.Bezier,reshapable: true, relinkableFrom: true, relinkableTo: true },
                                    $$(go.Shape),
    ...
    // En cliquant sur le bouton minico
    $("#minicoBtn").click(function() {
        var a = [], b = [], c = [], x = [];
        ...
        // x = minico(copieA, copieB, copieC);

        //  baseSolutionTable = x.map(function(arr) {
        //     return arr.slice();
        //  }); 
        
      baseSolutionTable = [[15,0,0,10,0,0],[0,15,0,0,15,0],[5,0,0,0,5,0],[0,0,35,0,0,10]];
    ...
    function minico(a, b, c) {
        var x = initMatrix(c);
	
	for( var j = 0; j< c[0].length; j++)
	{
		var colonne = getColonne(c,j);
		while(b[j] != 0)
		{
			var indexMin =indexOfMin(colonne,a);
    ...
    $("#bhammerBtn").click(function () {
        ...
        x = balas_hammer(copieA, copieB, copieC);
    ...
    function balas_hammer(a,b,tableauC){
    ...
    function getColonne(matrice, col){
       var column = [];
